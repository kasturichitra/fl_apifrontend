...
const BankAccountVerify = () => {
  const [response, setResponse] = useState(null);
  const navigate = useNavigate("");
   const [error, setError] = useState(false);
   const fetchWallet = useGetWallet();

  // Custom onChange handler to convert text to uppercase
  const handleOnChange = (e, setFieldValue) => {
    // Get the name of the field being updated
            ...
            <div className='d-flex justify-content-between align-items-center pe-4'>
                ...
                <div className='py-3 mt-3' style={{ borderTop: '1px solid black' }} >
                  ...
                  <div className='d-flex justify-content-between pe-3 my-3'>
                    <div className='text-secondary'> Setp 1: Enter AccountNumber</div> <div className='text-primary'> View More Details</div>
                  </div>
                  <div className='d-flex justify-content-between pe-3 my-3'>
                    <div className='text-secondary'> Setp 1: Enter IFSC Number</div> 
                  </div>
                  <div className='my-3'>
                    <b>Body :</b>
                  </div>
                  ...
                  onSubmit={async (values) => {
                    try {
                      ...
                      const apiResponse = await Call_Api(
                        "/account/verify-bank-account", // routes
                        "POST", // HTTP method
                        { account_no: values.accountNumber, ifsc: values.IFSC },
                        token
                      );
                  ...
                  >
                    ...
                    <pre style={{ marginTop: '10px', whiteSpace: 'pre-wrap' }}>
                      <code className="language-bash">
                        {EXResponse.BankAccountVerifyCurl}
                      </code>
                    </pre>
                    ...
                    <button
                      ...
                      onClick={() => navigator.clipboard.writeText(EXResponse.BankAccountVerifyCurl)}
                    >
                      📋 Copy
                    </button>
                  </div>
                ...
                <div className='ExResponse'>
                  ...
                  <div>
                    <div className='d-flex justify-content-between align-items-center'>
                      ...
                      <button className='btn btn-outline-secondary btn-sm' onClick={() => navigator.clipboard.writeText(JSON.stringify(EXResponse.BankAccountVerifyResponse, null, 2))}> 📋 Copy</button>
                    </div>
                  </div>
                  <SyntaxHighlighter language="json" style={atomOneDark} className="json-highlighter">
                    {JSON.stringify(EXResponse.BankAccountVerifyResponse, null, 2)}
                  </SyntaxHighlighter>
                </div>
              </div>
            </div>
