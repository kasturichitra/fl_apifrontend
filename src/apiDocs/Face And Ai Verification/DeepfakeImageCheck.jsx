...
import { DATA, DeepfakeImageCheckDynamic } from "../../utils/apiSchema";
import { api_Headers } from "../../utils/Api_Headers";
import { FetchApi } from "../../utils/Custom_Api";
import { FM, IC } from "../../utils/bodyParams";
import { GetAcc } from "../../utils/Language";

export default function DeepfakeImageCheck() {
  const [faceMatchState, setFaceMatchState] = useState({});
  const [apiResponse, setApiResponse] = useState(null);
  const [allRequiredFields, setAllRequiredFields] = useState({});

  const examplesList = GetAcc?.exampleCodes["DIC"] || [];
    ...
    <div className="main_parent">
      <div className="first_child hide-scrollbar">
        ...
        <div className="api_hero">
          <h1 className="api_heading">Deepfake Image Check</h1>

          <MethodLink
            method={"POST"}
            className={"method_link"}
            LinkClass={"link_class"}
            link={"image/deepfake_check"}
          />
        ...
        <div className="py-6">
          ...
          <ResponseComponent
            dynamic200={DeepfakeImageCheckDynamic}
            otherData={DATA}
          />
        </div>
      </div>
      ...
      <div className="second_child hide-scrollbar">
        <Codes
          ...
          service={"deepfakeImageCheck"}
          examples={GetAcc?.exampleCodes["DIC"] || []}
        />
      </div>
    </div>
  );
