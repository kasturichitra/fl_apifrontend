...
import { CAV, PNV } from "../../utils/bodyParams";
...
const CaVerification = () => {
  const [faceMatchState, setFaceMatchState] = useState({});
  const [apiResponse, setApiResponse] = useState(null);
  const [allRequiredFields, setAllRequiredFields] = useState({});

  const examplesList = GetAcc?.exampleCodes["CAV"] || [];
    ...
    <div className="main_parent">
      <div className="first_child hide-scrollbar">
        ...
        <div className="api_hero">
          <h1 className="api_heading">Ca Verification</h1>
          <MethodLink
            method="POST"
            className="method_link"
            LinkClass="link_class"
            link="professional/charteredAccount/verify"
          />
          <p className="first_para">
            The Chartered Accountant (CA) Verification API enables developers to
            verify the credentials and authenticity of registered CAs in
            real-time using their professional registration details, ensuring
      ...
      <div className="second_child hide-scrollbar">
        <Codes
          ...
          service={"caVerification"}
          examples={GetAcc?.exampleCodes["CAV"] || []}
        />
      </div>
    </div>
  );
